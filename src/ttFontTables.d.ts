...
// };
declare type KtOtjsGLookupListDesc =
{
  lookupType : KtOtjsGLSupportedDataTypeDict["USHORT"] ,
  lookupFlag: number ,
  subtables: {}[] ,
  markFilteringSet ?: KtOtjsGLSupportedDataTypeDict["USHORT"] ,
}

type KtOtjsGLSupportedDataTypeName = (import("./types").OtjsSupportedDataType )
type KtOtjsGLSupportedDataTypeEncode = ([typeof import("./types").encode] extends [infer T] ? T : never ) ;
type KtOtjsGLSupportedDataTypeDict = ([KtOtjsGLSupportedDataTypeEncode ] extends [infer T] ? ({ [k in keyof T]: Parameters<T[k]>[0] } ) : never ) ;
