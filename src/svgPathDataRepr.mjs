...
export function flipYBaseIfNecessary (commands, options )
{
...

  {
    ...
    return (
      ...
      commands
      .map((cmd, i) => /** @type {(([GpsbSplineSegmentCoord] extends [infer S] ? { [k in keyof [Required<S>][0]]: [k, S[k] ] }[keyof S] : never ) )[] } */ (Object.entries(cmd) ) )
      .map(cmdTable => (
        cmdTable
        .map(([prop, val0]) => {
          let val2 = val0 ;
          if (prop === 'x' || prop === 'x1' || prop === 'x2') {
            val2 = x + val2 * scale;
          }
          if (prop === 'y' || prop === 'y1' || prop === 'y2' ) {
            val2 = y + (flipY ? (flipYBase ?? throwMissingFlipBaseError(`y`) ) - val2 : val2 ) * scale;
          }
